...
subroutine zmq_req(socket_address, request, received_float)
   use iso_c_binding
   use zmq_client_module, only: zmq_client
   implicit none 
...
! 1. Open the socket
! module open_zmq_socket
!    use iso_c_binding 
!    implicit none 
!    interface 
!    ! function init_zmq_socket(socket_address) result(out_init) bind(C, name='init_zmq_socket')
!    !    use, intrinsic :: iso_c_binding
!    !    character(kind=c_char), dimension(*), intent(in) :: socket_address
!    !    type(c_float) :: out_init
!    ! end function init_zmq_socket
!     function init_zmq_socket(socket_address) bind(C, name='init_zmq_socket')
!         use, intrinsic :: iso_c_binding
!         character(kind=c_char), dimension(*), intent(in) :: socket_address
!         integer(c_int) :: init_zmq_socket  ! Assuming the C function returns an integer status
!     end function init_zmq_socket
! end interface
! end module open_zmq_socket
! 2. Send the message(s)
! module send_message_zmq
!    use iso_c_binding
!    implicit none 
!    interface 
!    function send_message(msg) result(out_comm) bind(C, name='send_message')
!       use, intrinsic                                   :: iso_c_binding 
!       character(kind=c_char), dimension(*), intent(in) :: msg 
!       type(c_ptr)                                      :: out_comm
!    end function send_message
!    end interface 
! end module send_message_zmq
! 3. Close the socket
! module close_zmq_socket
!    use iso_c_binding 
!    implicit none 
!    interface 
!    function end_zmq_socket() result(out) bind(C, name='end_zmq_socket')
!       use, intrinsic :: iso_c_binding
!       !character(kind=c_char), dimension(*), intent(in) :: socket_address
!       type(c_ptr) :: out 
!    end function close_zmq_socket
! end interface 
! end module close_zmq_socket
! --- Relative fortran subroutine to wrap modules 
! subroutine zmq_create(socket_address)
!    use iso_c_binding
!    use open_zmq_socket, only: init_zmq_socket 
!    implicit none 

!    character(*)                              :: socket_address 
!    real(c_int)                               :: response_code
!    ! real(c_float), pointer                    :: tmp_response 
!    ! character(c_int)                          :: response_code

!    response_code = init_zmq_socket(socket_address)
!    ! write(*, '(A, I0)') "ZMQ Socket creation status: ", response_code
!    ! if (c_associated(response_ptr)) then
!    !    call c_f_pointer(response_ptr, tmp_response)
!    ! end if 

!    ! write(received_status, '(I0)') tmp_response

!    ! Print the formatted string including code and channel information
!    ! print '(A, A, A, A)', "ZMQ Broadcaster socket opened with code ", &
!    !                         trim(adjustl(received_status)), " at ", trim(socket_address)
      
! end subroutine zmq_create
! --------------------------------------------------- !
! subroutine broadcast(message) 
!    use iso_c_binding
!    use send_message_zmq, only: send_message
!    implicit none 

!    character(*)            :: message 
!    type(c_ptr)             :: response_ptr_message 
!    real(c_float), pointer  :: tmp_response_message
!    character(c_char)       :: received_status_message

!    response_ptr_message = send_message(message) 

!    if (c_associated(response_ptr_message)) then
!       call c_f_pointer(response_ptr_message, tmp_response_message)
!    end if 

!    write(received_status_message, '(I0)') tmp_response_message
! end subroutine broadcast
! --------------------------------------------------- !
! subroutine zmq_destroy()
!    use iso_c_binding
!    use close_socket, only: end_zmq_socket 
!    implicit none 

!    type(c_ptr)                               :: response_ptr_end
!    real(c_float), pointer                    :: tmp_response_end
!    character(c_char)                         :: received_status_end

!    response_ptr_end = end_zmq_socket()

!    if (c_associated(response_ptr_end)) then
!       call c_f_pointer(response_ptr_end, tmp_response_end)
!    end if 

!    write(received_status_end, '(I0)') tmp_response_end

!    ! Print the formatted string including code and channel information
!    print '(A, A, A, A)', "ZMQ Broadcaster socket closed with code ", trim(adjustl(received_status_end)) 
      
! end subroutine zmq_destroy
...
SUBROUTINE FAST_InitializeAll_T( t_initial, TurbID, Turbine, ErrStat, ErrMsg, InFile, ExternInitData )
   ...
   character(*), parameter                          :: broadcast_socket_address =  "tcp://127.0.0.1:8888" // c_null_char

   Turbine%TurbID = TurbID

   ! call zmq_create(broadcast_socket_address)
...
!   call initialize_socket() ! opening socket for broadcasting live fast data 
...
SUBROUTINE FAST_Solution(t_initial, n_t_global, p_FAST, y_FAST, m_FAST, ED, BD, SrvD, AD14, AD, IfW, OpFM, SC_DX, HD, SD, ExtPtfm, &
   ...
   ! ZMQ - related definitions --------------------------------------------------
   character(*), parameter                 :: socket_address = "tcp://127.0.0.1:5555" // c_null_char
   character(*), parameter                 :: request        = "wspeed" // c_null_char
   real(c_float)                           :: received_float
   ! -----------------------------------------------------------------------------
   ! -----------------------------------------------------------------------------
...
!...............................................................................................................................
   ...
   integer                                 :: i
   character(len=50)                       :: message             ! message to be sent to zmq
   ErrStat = ErrID_None
   ErrMsg  = ""
  ...
  !y_FAST%WriteThisStep = NeedWriteOutput(n_t_global, t_global, p_FAST)
   IF ( y_FAST%WriteThisStep )  THEN
         ...
         ! ------------------------ ZMQ broadcasting -------------------------------- !
         ! write(message, '(A, F10.6)') "time = ", t_global
...
SUBROUTINE WrOutputLine( t, p_FAST, y_FAST, IfWOutput, OpFMOutput, EDOutput, y_AD, SrvDOutput, HDOutput, SDOutput, ExtPtfmOutput,&
   ...
   character(len=50)                :: message             ! message to be sent to zmq
   integer                          :: i                   ! loop counter for comm
   character(len=10) :: intStr  ! Declare a character variable to hold the integer as string
   ...
   ! ------- Cheating w/ ZMQ communication. Select only inputs for comm in Fast files ----- !
   ! write(message, '(A, F10.3)') " WindVelX = ", IfWOutput(1)
   ! call broadcast(message)
   ! write(message, '(A, F10.3)') " WindVelY = ", IfWOutput(2)
   ! call broadcast(message)
   ! write(message, '(A, F10.3)') " WindVelZ = ", IfWOutput(3)
   ! call broadcast(message)
   ! ! SrvD
   ! write(message, '(A, F10.3)') " GenTrq (kN m) = ", SDOutput(1)
   ! call broadcast(message)
   ! write(message, '(A, F10.3)') " GenPwr (W)    = ", SDOutput(2)
   ! call broadcast(message)
   ! do i = 1, 3
   !       write(intStr, '(I1)') i  
   !       write(message, '(A, A, F10.3)') "BlPitchC", adjustl(trim(intStr)), " (kN m)= ",SDOutput(2 + i)
   !       call broadcast(message)
   ! end do
   ! ------------------------------------------------------------------------------------ !
...
SUBROUTINE FillOutputAry(p_FAST, y_FAST, IfWOutput, OpFMOutput, EDOutput, y_AD, SrvDOutput, HDOutput, SDOutput, ExtPtfmOutput, &
   ...
   !    ! y1: GenTrq
   !    write(message, '(A, F10.3)') "GenTrq (kN m)= ", SDOutput%GenTrq
   !    call send_message_from_fortran(message)
   !    ! y2: HSS Brake torque command 
   !    write(message, '(A, F10.3)') "HSSBrTqC (kN m)= ", SDOutput%HSSBrTqC
   !    call send_message_from_fortran(message)
   !    ! y3,4,5: Blade pitch commands
   !    do i = 1, 3
   !       write(message, '(A, A, F10.3)') "BlPitchC", adjustl(i), " (kN m)= ", SD%y%BlPitchCom(i)
   !       call send_message_from_fortran(message)
   !   end do
   !   ! y6, 7, 8: Measured hub wind speed
   !   write(message, '(A, F10.6)') "WindHubVelX (m/s) = ", IF%y%WindHubVelX
   !   call send_message_from_fortran(message)
   !   write(message, '(A, F10.6)') "WindHubVelY (m/s) = ", IF%y%WindHubVelY
   !   call send_message_from_fortran(message)
   !   write(message, '(A, F10.6)') "WindHubVelZ (m/s) = ", IF%y%WindHubVelZ
   !   call send_message_from_fortran(message)
   !   ! y9: azimuth angle of blade 1
   !   write(message, '(A, F10.6)') "B1Azimuth (deg) = ", AD%y%B1Azimuth
   !   call send_message_from_fortran(message)
   !   ! y10: rotor tsr 
   !   write(message, '(A, F10.6)') "RtTSR (-) ", AD%y%RtTSR
   !   call send_message_from_fortran(message)
   !   ! y11: rotor speed
   !   write(message, '(A, F10.6)') "RtSpeed (rpm) = ", AD%y%RtSpeed 
   !   call send_message_from_fortran(message)
   !   ! y12, y13, 14: "measured" aerodynamic coefficients
   !   write(message, '(A, F10.6)') "RtAeroCp (-) = ", AD%y%RtAeroCp
   !   call send_message_from_fortran(message)
   !   write(message, '(A, F10.6)') "RtAeroCq (-) = ", AD%y%RtAeroCq
   !   call send_message_from_fortran(message)
   !   write(message, '(A, F10.6)') "RtAeroCt (-) = ", AD%y%RtAeroCt
   !   call send_message_from_fortran(message)
   !   ! y15: Rotor aerodynamic power
   !   write(message, '(A, F10.6)') "RtAeroPwr (W) = ", AD%y%RtAeroPwr
   !   call send_message_from_fortran(message)
   !   ! y16: Rotor yaw angle
   !   write(message, '(A, F10.6)') "YawPzn (deg) = ", ED%y%YawPzn
   !   call send_message_from_fortran(message)
   !   write(message, '(A, F10.6)') "YawVzn (deg/s) = ", ED%y%YawVzn
   !   call send_message_from_fortran(message)
   !   ! y17, 18, 19: bending moments at shaft tip
   !   write(message, '(A, F10.6)') "LSSTipMya (W) = ", ED%y%LSSTipMya
   !   call send_message_from_fortran(message)
   !   write(message, '(A, F10.6)') "LSSTipMxa (W) = ", ED%y%LSSTipMxa
   !   call send_message_from_fortran(message)
   !   write(message, '(A, F10.6)') "LSSTipMza (W) = ", ED%y%LSSTipMza
   !   call send_message_from_fortran(message)
   !   ! y20: electrical generator powre
   !   write(message, '(A, F10.6)') "LSSTipMya (W) = ", SD%y%GenPwr
   !   call send_message_from_fortran(message)
   !   ! ------------------------------------- end of zmq broadcasting ----------------------!
