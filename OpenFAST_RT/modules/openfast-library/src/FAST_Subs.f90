...
! -------------------------------------------------------------------------------------------------------------------------------
! ---------------------- ZMQ Broadcaster module ---------------------------
! module ZMQ_Broadcaster 
!    interface 
!    function init_socket()  result(out) bind(c, name='init_socket')
!       !print *, "Broadcasting socket initialized at localhost port n. 8888"
!    end function init_socket
!    end interface 

!    interface 
!    function send_message(msg) bind(c, name='send_message')
...
!    interface 
!    function close_socket() bind(c, name='close_socket')
!    end function close_socket
!    end interface
! end module ZMQ_Broadcaster

! ! integer, parameter :: max_message_length = 1024
! ! end module ZMQ_Broadcaster

! ! ------------------------------------------------------------------------
! subroutine init_socket_sr()
!    !use iso_c_binding
!    use ZMQ_Broadcaster, only: init_socket
!    implicit none 

!    call init_socket()

! end subroutine init_socket_sr
! ! ------------------------------------------------------------------------
! subroutine send_message_sr(message)
!    use iso_c_binding
!    use ZMQ_Broadcaster, only: send_message
!    implicit none 
!    type(c_str) :: message

!    call send_message(message)

! end subroutine send_message_sr
! ! -----------------------------------------------------------------------
! subroutine close_socket_sr()
!    use iso_c_binding
!    use ZMQ_Broadcaster, only: close_socket
!    implicit none 

!    call close_socket()

! end subroutine close_socket_sr
! ! ----------------------------------------------------------------------
! subroutine send_message_from_fortran(message)
!    character(*), intent(in) :: message 
!    call send_message_sr(trim(message))
...
module open_zmq_socket
   ...
   implicit none 
   interface 
   function init_zmq_socket(socket_address) result(out) bind(C, name='init_zmq_socket')
      use, intrinsic :: iso_c_binding
...
subroutine zmq_create(socket_address)
   ...
   character(c_char)                 :: received_status

   response_ptr = init_zmq_socket(socket_address)
...
! subroutine initialize_socket 
!    use iso_c_binding
!    use zmq_wrapper_interface, only: init_socket
!    implicit none 
